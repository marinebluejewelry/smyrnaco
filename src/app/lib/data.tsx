...
const siteContent: SiteContent = {
  ...
  howWeDoIt: {
    ...
    steps: [
      ...
      {
        id: "step-3d-sampling",
        tabLabel: "3D Sampling",
        headline: "Form Before Metal",
        body: <>Before any precious material is committed, every design must prove itself in three dimensions. We translate our finalised digital models into physical samples using high-resolution 3D printing — a process that bridges the gap between screen and hand. Each printed prototype is examined for proportion, weight distribution, and the subtle interplay of surfaces that no render can fully predict. A charm that reads beautifully on a monitor may feel clumsy between the fingers; a clasp that looks elegant in isolation may sit awkwardly against the wrist. These are truths that only a physical sample can reveal.<br/><br/>Iteration at this stage is deliberate and unhurried. Samples are worn, compared, adjusted, and reprinted until every contour resolves into something that feels both precise and effortless. The cost of reprinting a polymer prototype is negligible compared to the cost of discovering a flaw after casting — so we print freely, discard generously, and approve cautiously. Only when a sample survives this scrutiny does it earn the right to move forward into mold-making, carrying with it the confidence that its form has been tested against reality rather than assumption.</>,
        videoSrc: videoPath("how-we-do-3d-sampling.mp4"),
        posterSrc: "/images/how-we-do-3d-sampling.webp",
      },
      {
        id: "step-wax-molding",
        tabLabel: "Wax Molding",
        headline: "Pattern & Mold",
        body: <>Once a 3D-printed sample has been approved, it becomes the master pattern from which all future reproductions will descend. The sample is carefully embedded in a vulcanised rubber mold compound — a process that captures every surface detail, every undercut, every micro-texture with remarkable fidelity. When the mold is opened and the master removed, what remains is a precise negative cavity: the inverse shape of the original, ready to receive molten wax.<br/><br/>Wax injection follows — liquid wax is pressed into the rubber mold under controlled temperature and pressure, producing a faithful wax replica of the original sample. These wax copies are then assembled onto a central wax column, forming what the industry calls a tree: a branching structure that may carry dozens of identical pieces. The tree is an elegant solution to the problem of scale — it allows multiple components to be cast simultaneously while ensuring each one receives an even flow of metal. Building a clean, balanced tree is itself a craft, requiring an understanding of how molten metal moves through narrow channels and around complex geometries.</>,
        videoSrc: videoPath("how-we-do-wax-molding.mp4"),
        posterSrc: "/images/how-we-do-wax-molding.webp",
      },
      {
        id: "step-casting",
        tabLabel: "Casting",
        headline: "From Wax to Metal",
        body: <>Casting is the moment of transformation — the stage where a design leaves the provisional world of wax and becomes permanent in metal. The assembled wax tree is encased in a ceramic investment material that hardens around it, forming a heat-resistant shell. The entire assembly is then placed in a kiln, where rising temperatures melt and burn away the wax completely, leaving behind a hollow cavity that mirrors the original forms with extraordinary precision. This ancient technique, known as lost-wax casting, has been practised for over five thousand years; we use the same fundamental principle, refined by modern temperature control and alloy science.<br/><br/>Molten metal — heated to the exact temperature required by the specific alloy — is poured or forced into the evacuated mold under vacuum or centrifugal pressure. The metal fills every channel, every detail, every surface texture that the wax once occupied. Once cooled, the ceramic shell is broken away to reveal a metal tree: a cluster of raw castings still attached to their central column. Each piece is then carefully cut from the tree, cleaned, and inspected before it moves on to finishing. The transition from wax to metal is irreversible and unforgiving — which is precisely why every preceding step exists to ensure that only proven forms reach this point.</>,
        videoSrc: videoPath("how-we-do-casting.mp4"),
        posterSrc: "/images/how-we-do-casting.webp",
      },
      {
        id: "step-plating",
        tabLabel: "Plating",
